...
if( $_SERVER['REQUEST_METHOD']=='GET' )
	{	$where = Array();
		...
		$conformidade = $_GET['x'];
		$palavra = $_GET['p'];
		$observdisplay = $_GET['b'];
		
if ($observdisplay == "none") {$observdisplaymostra = "style='display:none'";} else {$observdisplaymostra = "";};
...
$data6 = $_GET['data6'];
		...
		$conform = $_GET['x']; switch ($conform) {case "conf": $traco = "AND"; break; case "inconf": $traco = "OR"; break;}
		
		if( $atividade ){ $where[] = " tipo = '{$atividade}'"; }
		if( $supervisor ){ $where[] = " nomesup = '{$supervisor}'"; }
		if( $cliente ){ $where[] = " nomeposto = '{$cliente}'"; }
		if( $conformidade ){ $where[] = "rotina = '{$conformidade}' $traco postura = '{$conformidade}' $traco uniforme = '{$conformidade}' $traco ocorrencia = '{$conformidade}' $traco cartao = '{$conformidade}' $traco pastamanual = '{$conformidade}' $traco equipamento = '{$conformidade}' $traco epi = '{$conformidade}' $traco estoqueprod = '{$conformidade}' $traco cliente = '{$conformidade}' $traco outros = '{$conformidade}'"; }
		if( $palavra ){ $where[] = " observacao  LIKE '%$palavra%'"; }
		if( $inicio ){ $where[] = " data BETWEEN '{$inicio}' and '{$final}'"; }}
...
print "<br /><b>".$totalreg."</b> Resultados do filtro<br />
<br />
