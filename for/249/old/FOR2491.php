...
echo "<table class='table table-hover table-condensed' style='margin-bottom:0'>
	<tr>
    <td class='col-md-5  col-sm-5 col-xs-8 text-left'>".nl2br($row['campo4'])."</td>
...
<?php
/*select id, campo4, campo8 from for05412 UNION select id, campo4, campo8 from for054fim GROUP BY campo8 ORDER BY campo4*/
$result=mysql_query("SELECT * FROM (SELECT * FROM for05412 ORDER BY campo4, campo8, data2 DESC) AS a GROUP BY a.campo19, campo4 order by campo4");
while($row=mysql_fetch_array($result))
echo "<table class='table table-hover table-condensed' style='margin-bottom:0'>
	<tr>
    <td class='col-md-5  col-sm-5 col-xs-8 text-left'>".nl2br($row['campo4'])."</td>
