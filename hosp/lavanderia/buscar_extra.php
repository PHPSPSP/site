...
$consulta2 = mysql_query("SELECT data_hora FROM lavanderia_extra WHERE lavanderia_local = '".$l."' AND lavanderia_cliente = '".$clienteativo."' ORDER BY lavanderia_id_extra DESC limit 1");
$dados2 = mysql_fetch_array($consulta2);
$ultimo=$dados2['data_hora'];

$consulta = mysql_query("SELECT e.lavanderia_id_enxoval, e.lavanderia_nome_enxoval, e.lavanderia_cliente_enxoval, c.lavanderia_local, c.lavanderia_enxoval, c.contagem_entrega, c.lavanderia_id_extra, c.data_hora FROM lavanderia_extra c, lavanderia_enxoval e where e.lavanderia_cliente_enxoval = '$clienteativo' AND c.lavanderia_local='$l' and e.lavanderia_id_enxoval = c.lavanderia_enxoval AND c.data_hora = '".$ultimo."'");
