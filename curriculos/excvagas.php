...
if ($_SESSION['tipo'] != "adm" && $_SESSION['tipo'] != "rh"){
	  echo ("<SCRIPT LANGUAGE='JavaScript'>
        window.alert('Este usuario nao e autorizado a deletar vagas!');
		window.close();
        </SCRIPT>");}else{	};
    ...
    <div class="row">
      <div class="col-md-12">
        <header class="text-center element-normal-top element-medium-bottom not-condensed os-animation" data-os-animation="fadeInUp" data-os-animation-delay="0s">
          ...
          <h1 class="super hairline bordered bordered-normal"> Vagas Cadastradas </h1>
        </header>
        <div class="row">
          <div class="col-md-12">

           <?php
$result=mysql_query("select * from vagas ORDER BY regiao, funcao");
echo "
 <div class='element-short-top element-short-bottom os-animation animated fadeIn table-responsive' data-os-animation='fadeIn' data-os-animation-delay='0s' style='animation-delay: 0s;'>    <table class='table table-hover table-condensed'><thead>
        <tr>
            ...
            <th class='col-md-3  col-sm-3 col-xs-3 text-center'>Função</th>
            <th class='col-md-5  col-sm-5 col-xs-5 text-center'>Observação</th>
            <th class='col-md-2  col-sm-2 col-xs-2 text-center'>Ação</th>
            </tr>
</thead>
              </table>
            </div>
...
<tr>
            <td class='col-md-2  col-sm-2 col-xs-2 tex'>".$row['regiao']."</td>
            <td class='col-md-3  col-sm-3 col-xs-3 tex'>".$row['funcao']."</td>
            <td class='col-md-5  col-sm-5 col-xs-5 tex'>".$row['observacao']."</td>
            <td class='col-md-2  col-sm-2 col-xs-2 text-center'><a class='btn btn-danger' href='javascript:confirmExcluir(\"deletar3.php?id=" . $row['id'] . "\");'>Deletar</a></td>
        </tr>
    </table>
</div>
";
...
function confirmExcluir(delUrl) {
  if (confirm("Deseja realmente excluir essa vaga?")) {
